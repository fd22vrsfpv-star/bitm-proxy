#!/bin/bash
# Stop any locally-running bitm-proxy processes (Python control plane, Go
...
echo "==> Inspecting"
BEFORE="$(pgrep -af 'backend\.run|'"$SCRIPT_DIR"'/\.local/bitm-proxies|headless_shell|playwright.*chromium' 2>/dev/null || true)"
if [ -z "$BEFORE" ]; then
    echo "   No local bitm-proxy processes found."
else
    echo "$BEFORE"
fi
...
pkill -TERM -f "$SCRIPT_DIR/\.local/bitm-proxies" 2>/dev/null || true
sleep 2

# Step 2: force (SIGKILL) for anything still around
pkill -KILL -f 'backend\.run' 2>/dev/null || true
pkill -KILL -f "$SCRIPT_DIR/\.local/bitm-proxies" 2>/dev/null || true
pkill -KILL -f 'headless_shell' 2>/dev/null || true
pkill -KILL -f 'playwright.*chromium' 2>/dev/null || true

echo
echo "==> Re-checking"
AFTER="$(pgrep -af 'backend\.run|'"$SCRIPT_DIR"'/\.local/bitm-proxies|headless_shell|playwright.*chromium' 2>/dev/null || true)"
if [ -z "$AFTER" ]; then
    echo "   All stopped."
else
...
echo
echo "==> Port listeners on bitm-proxy ports"
